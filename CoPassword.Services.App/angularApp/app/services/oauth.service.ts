...
export class OAuthService {
    ...
    tryLogin(options) {
        ...
        const strAccess = sessionStorage.getItem('str_access');
        const parts = this.getFragment(strAccess);
        ...
        */
        ...
        if (this.clearHashAfterLogin) {
            location.hash = '';
            this.router.navigate(['/vaults'], {queryParams: {'type': 'user'}});
        }
    ...
    logOut(noRedirectToLogoutUrl = false) {
        ...
        if (this.logoutUrl.indexOf('{{') > -1) {
            ...
            logoutUrl = this.logoutUrl + '?id_token_hint='
                ...
                + '&acr_values='
                + encodeURIComponent(this.acr_values)
        }
    ...
    getFragment(str) {
        const url = str ? str : window.location.hash.substr(1);
        sessionStorage.removeItem('str_access');
        return this.parseQueryString(url);
    };
    ...
    checkAtHash(accessToken, idClaims) {
        ...
        atHash = tokenHashBase64.replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
